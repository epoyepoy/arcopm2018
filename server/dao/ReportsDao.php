	...
	public function getMyReportingLine($filters)
	{
	 $queryString = "
		...
		Declare @evaluator as varchar(5) = (
		 ...
		 SELECT CASE WHEN COALESCE(RLE.STATE,RL.STATE)=5 THEN 2
		 WHEN COALESCE(RLE.STATE,RL.STATE)=4 THEN 4 END AS ReportingOrder,0,  CASE WHEN COALESCE(RLE.STATE,RL.STATE)=5 THEN 'MY EVALUATOR'
					 WHEN COALESCE(RLE.STATE,RL.STATE)=4 THEN 'MY DOTTED LINE MANAGER' END as ParentRelation, '' AS ParentID, COALESCE(RLE.STATE,RL.STATE) AS relState, CASE WHEN COALESCE(RLE.STATE,RL.STATE)=5 THEN 'MY EVALUATOR'
					 WHEN COALESCE(RLE.STATE,RL.STATE)=4 THEN 'MY DOTTED LINE MANAGER'
		 ...
		 --get reporting to me
		 UNION
		 SELECT 5 AS ReportingOrder, 0, CASE WHEN COALESCE(RLE.STATE,RL.STATE)=5 THEN 'EVALUATOR'
		 WHEN COALESCE(RLE.STATE,RL.STATE)=4 THEN 'DOTTED LINE MANAGER'
	 	 END as 'ParentRelation',  COALESCE(RLE.empnotarget,RL.empnotarget) AS ParentID, COALESCE(RLE.STATE,RL.STATE) AS relState, CASE WHEN COALESCE(RLE.STATE,RL.STATE)=5 THEN 'EVALUATOR'
					 WHEN COALESCE(RLE.STATE,RL.STATE)=4 THEN 'DOTTED LINE MANAGER'
				 END as 'RelationshipDesc', VEM.empno as 'empNo', rtrim(ltrim(VEM.family_name))+' - '+rtrim(ltrim(VEM.first_name)) as 'empName', VEM.job_desc as 'empPosition',
		 ...
		 WHERE ERL.empnotarget=VEM.empno AND ERL.state=5 AND ERL.excludeFromCycles<>@CycleID
		 ) assigned
		 OUTER APPLY (
		 SELECT COUNT(*) AS Completed
		 FROM dbo.ReportingLine ERL
		 INNER JOIN dbo.Evaluations EE ON ERL.empnosource=EE.EmployeeID 
		 WHERE ERL.empnotarget=VEM.empno AND ERL.state=5 AND ERL.excludeFromCycles<>@CycleID AND ee.State in (6,7) AND EE.CycleID=@CycleID
		 ) completed 
		 OUTER APPLY (
			SELECT COUNT(*) AS Calibrated
			FROM dbo.ReportingLine ERL
			INNER JOIN dbo.Evaluations EE ON ERL.empnosource=EE.EmployeeID 
			WHERE ERL.empnotarget=VEM.empno AND ERL.state=5 AND ERL.excludeFromCycles<>@CycleID AND ee.State in (7) AND EE.CycleID=@CycleID
			) calibrated
		 WHERE (COALESCE(RLE.empnotarget, Rl.empnotarget)=@empno) AND (COALESCE(RLE.State, RL.state) in (4,5)) AND (COALESCE(RLE.goalCycle, RL.excludeFromCycles) <> @CycleID) 
		 
		 UNION

		 SELECT 6 AS ReportingOrder, chkRelationship.state, CASE WHEN  COALESCE(RLE.STATE,RL.STATE) = 5 AND chkRelationship.state=5 THEN 'REVIEWER' ELSE 'REV DOTTED' END as ParentRelation,  
		 COALESCE(RLE.empnotarget,RL.empnotarget) AS ParentID, 
		 COALESCE(RLE.STATE,RL.STATE) AS relState, CASE WHEN chkRelationship.state=4 THEN 'EVALUATOR' WHEN COALESCE(RLE.STATE,RL.STATE)=5 THEN 'REVIEWER'
					 WHEN COALESCE(RLE.STATE,RL.STATE)=4 THEN 'DOTTED LINE MANAGER'
				 END as 'RelationshipDesc', VEM.empno as 'empNo', rtrim(ltrim(VEM.family_name))+' - '+rtrim(ltrim(VEM.first_name)) as 'empName', VEM.job_desc as 'empPosition',
		 VEM.family_desc as 'empDepartment', VEM.pay_cs as 'empSite', VEM.site_desc as 'empSiteDesc', assigned.totalAssigned AS AssignedEvaluations, completed.Completed AS CompletedEvaluations, calibrated.Calibrated
		 FROM  vw_arco_employee VEM
		...
		SELECT VEM.empno as 'empNo'
		 ...
		 ) AND (COALESCE(RLE.State, RL.state) in (4,5)) AND (COALESCE(RLE.goalCycle, RL.excludeFromCycles) <> @CycleID) 
 ...
 */
	...
	public function getEvaluatorReportingLine($filters)
	{
	 $queryString = "
	 	DECLARE @empno as varchar(5) = :empno;
		DECLARE @CycleID AS INT =:cycleid;
		SELECT 'Reporting to me:' AS ReportingType,COALESCE(RLE.STATE,RL.STATE) AS State, CASE WHEN COALESCE(RLE.STATE,RL.STATE)=5 THEN 'EVALUATOR'
		WHEN COALESCE(RLE.STATE,RL.STATE)=4 THEN 'DOTTED LINE MANAGER'
			END as 'RelationshipDesc', VEM.empno as 'empNo', rtrim(ltrim(VEM.family_name))+' - '+rtrim(ltrim(VEM.first_name)) as 'empName', VEM.job_desc as 'empPosition',
		...
		FROM dbo.ReportingLine ERL
		INNER JOIN dbo.Evaluations EE ON ERL.empnosource=EE.EmployeeID 
		WHERE ERL.empnotarget=VEM.empno AND ERL.state=5 AND ERL.excludeFromCycles<>@CycleID AND ee.State=6 AND EE.CycleID=@CycleID
		) completed 
		WHERE (COALESCE(RLE.empnotarget, Rl.empnotarget)=@empno) AND (COALESCE(RLE.State, RL.state) in (4,5)) AND (COALESCE(RLE.goalCycle, RL.excludeFromCycles) <> @CycleID) 
