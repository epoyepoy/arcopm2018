...
class GoalsDAO{
	...
	{
		$queryString = "
	   DECLARE @cycleid as int=:cycleid, @grade as varchar(2),@empno as varchar(5)=:empno, @userid as varchar(5) =:userid, @evalid int, @evalstate int, @site as varchar(5), @position as varchar(5);
	   BEGIN
		   SET NOCOUNT ON
		   BEGIN TRY
		   ...
		   SELECT @evalid = EvaluationID, @evalstate=State FROM Evaluations WHERE CycleID=@cycleid AND EmployeeID= @empno;
			   ...
			   IF (ISNULL(@evalid,0)=0) BEGIN
				   ...
				   SELECT @grade=grade, @site=pay_cs, @position=post_title_code from vw_arco_employee where empno=@empno;

				   INSERT INTO dbo.Evaluations
				   OUTPUT INSERTED.EvaluationID
				   VALUES (@cycleid,  @empno, @grade, 0, getdate(), NULL, @userid, NULL, NULL, @site, @position);
